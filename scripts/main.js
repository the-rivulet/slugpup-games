...
export function addPup() {
    ...
    btn.onclick = function () {
        let statsElem = btn.parentElement.getElementsByClassName("pupstats")[0];
    ...
    btn2.onclick = function () {
        let statsElem = btn2.parentElement.getElementsByClassName("pupstats")[0];
        let bars = Array.from(statsElem.getElementsByClassName("statbar"));
        for (let i of bars) {
            ...
            let txt = i.parentElement.getElementsByClassName("txt")[0];
    ...
    picker.onchange = function () {
        pupUI.getElementsByClassName("pupimg")[0].style.background = picker.value;
        Array.from(pupUI.getElementsByClassName("statbar")).forEach(x => { x.style.background = picker.value; });
    };
    ...
    for (let i of ["they", "them", "their"]) {
        ...
        if (i != "their")
            customPrns.insertAdjacentText("beforeend", " / ");
    }
    ...
    prns.onchange = function () {
        if (prns.value == "custom")
            customPrns.style.opacity = "1";
        else
            customPrns.style.opacity = "0";
    };
    ...
    Array.from(pupUI.querySelectorAll('input[type="range"]')).forEach(x => { x.style.background = color; });
...
export function beginGame() {
    try {
        ...
        for (let i of pups) {
            let statbars = Array.from(i.getElementsByClassName("statbar"));
            let pup = new Slugpup();
            pup.color = i.getElementsByClassName("statbar")[0].style.background;
            pup.name = i.getElementsByClassName("pupname")[0].value || "an unnamed slugpup";
            let prns = i.getElementsByClassName("pronouns")[0].value;
            if (prns == "custom") {
                let they = i.getElementsByClassName("they")[0].value || "they";
                let them = i.getElementsByClassName("them")[0].value || "them";
                let their = i.getElementsByClassName("their")[0].value || "their";
            ...
            for (let i of Object.keys(Stat).slice(0, 6)) {
                pup.stats[Stat[i]] = statbars[x].value;
                x++;
            }
            Game.pups.push(pup);
        }
