...
import { getId, Stat, Action, Item, shuffle, Relation, DeathCause, RelationType, Perk, Creature, randomColor } from "./globals.js";
...
export class Game {
    ...
    static actionPool: Action[] = [
        ...
        {pups: 2, text: "[0] looks for some easy kills.", chance: 0.1, bias: t => t[0].bias(Stat.aggression, 0.2) + t[0].bias(Stat.sympathy, -0.1)},
        ...
        // Relation Idle
        {pups: 2, text: "[0] and [1] explore the region together.", chance: 0.1, req: t => t[0].relationWith(t[1]).shared == RelationType.friends},
        {pups: 2, text: "[0] confides in [1] about their fear of death.", chance: 0.1, bias: t => t[0].bias(Stat.nervousness, 0.1), req: t => t[0].relationWith(t[1]).shared == RelationType.friends},
        ...
        {pups: 1, text: "[0] can't find the strength to go on.", chance: 0.2, req: t => t[0].perks.includes(Perk.suicidal)},
        // Food
        ...
        {pups: 1, text: "[0] comes across some blue fruit and eats it.", chance: 0.1, bias: t => t[0].bias(Stat.dominance, 0.1), effect: t => {t[0].xp += Math.floor(Math.random() * 2) + 2;}},
        // Friendships
        {pups: 2, text: "[0] asks [1] if they can be friends. [1] gratefully accepts.", chance: 0.4, bias: t => t[0].friend(0.4) + t[1].friend(0.4), req: t => !t[0].anyRelation(t[1], RelationType.friends, RelationType.swornEnemies), effect: t => {t[0].become(RelationType.friends, t[1], true);}},
        ...
        {pups: 2, text: "[0] wants to be friends with [1], but [1] politely declines.", chance: 0.3, bias: t => t[0].friend(0.3) + t[1].friend(-0.3) + t[1].bias(Stat.nervousness, 0.2), req: t => !t[0].anyRelation(t[1], RelationType.friends, RelationType.swornEnemies)},
        ...
        // Enemyships?
        {pups: 2, text: "[0] decides they hate [1] and will do anything to kill them.", chance: 0.4, bias: t => t[0].friend(-0.4), req: t => t[0].relationWith(t[1]).forth != RelationType.friends && t[0].relationWith(t[1]).forth != RelationType.swornEnemies, effect: t => {t[0].become(RelationType.swornEnemies, t[1]);}},
        ...
        {pups: 1, text: "[0] starts to question their own sanity.", chance: 0.1, bias: t => t[0].bias(Stat.nervousness, 0.1), req: t => !t[0].perks.includes(Perk.suicidal), effect: t => {if(Math.random() < 0.5) t[0].perks.push(Perk.suicidal);}},
        {pups: 1, text: "[0] just wants this all to be over.", chance: 0.1, bias: t => t[0].bias(Stat.energy, -0.1), req: t => !t[0].perks.includes(Perk.suicidal), effect: t => {if(Math.random() < 0.5) t[0].perks.push(Perk.suicidal);}},
        ...
        {pups: 1, text: "[0] finds another lizard and convinces it to join their army.", chance: 0.1, bias: t => t[0].bias(Stat.sympathy, 0.3), req: t => t[0].perks.includes(Perk.tamedLizard), effect: t => {t[0].perks.push(Perk.tamedLizard);}},
        // Unperks
        ...
        // Scavs
        {pups: 1, text: "[0] trades their pearl for a scavenger's grenade.", chance: 0.6, bias: t => t[0].perks.includes(Perk.highRep) ? 0.9 : 0, req: t => t[0].item == Item.pearl && !t[0].perks.includes(Perk.lowRep), effect: t => {t[0].item == Item.grenade; if(Math.random() < 0.5 && !t[0].perks.includes(Perk.highRep)) t[0].perks.push(Perk.highRep);}},
        {pups: 2, text: "[0] purchases spears for themselves and their friend [1] with a pearl.", chance: 0.8, bias: t => t[0].perks.includes(Perk.highRep) ? 1.2 : 0, req: t => t[0].item == Item.pearl && t[0].relationWith(t[1]).forth == RelationType.friends && !t[0].perks.includes(Perk.lowRep),
            effect: t => {t[0].item == Item.spear; t[1].item == Item.spear; if(Math.random() < 0.5 && !t[0].perks.includes(Perk.highRep)) t[0].perks.push(Perk.highRep); if(Math.random() < 0.3 && !t[1].perks.includes(Perk.highRep)) t[1].perks.push(Perk.highRep);}},
        {pups: 1, text: "[0] throws a spear into a scavenger toll, killing one.", chance: 0.3, bias: t => t[0].attack(0.3), req: t => t[0].item == Item.spear && !t[0].perks.includes(Perk.highRep), effect: t => {t[0].item = Item.empty; t[0].xp += 6; if(!t[0].perks.includes(Perk.lowRep)) t[0].perks.push(Perk.lowRep);}},
        ...
        // Unarmed Kills
        {pups: 2, text: "[0] challenges [1] to a brawl and comes out victorious.", chance: 0.3, bias: t => t[0].bias(Stat.bravery, 0.4) + t[0].attack(0.2, 0.2, t[1]) + t[1].attack(0.2), req: t => t[0].item == Item.empty && t[1].item == Item.empty, effect: t => {t[1].die(DeathCause.brawl, t[0]);}},
    ...
    static nextRound() { try {
        ...
        for(let i of order) {
            ...
            if(i.action.pups.length < i.action.action.pups) alert("uh oh! action '" + i.action.action.text + "' got " + i.action.pups.length + "/" + i.action.action.pups);
        ...
        setTimeout(function() { try {
            ...
            for(let i of order) {
                ...
                for(let j of i.action.pups) {
                    ...
                    txt.innerHTML = Object.keys(i.stats).map(x => x + " - " + i.stats[x]).join("<br/>");
            ...
            for(let i of Game.pups.sort((a, b) => b.xp - a.xp)) {
                ...
                let text = document.createElement("span");
                if(i.dead) text.style.color = "#bbb";
                text.classList.add("scoretext");
                text.textContent = `${i.name} - ${i.xp} XP`;
                if(i.dead && !i.killedBy && !i.deathCause) text.insertAdjacentText("beforeend", " - dead");
                if(!i.dead) text.insertAdjacentText("beforeend", " - alive" + (i.perks.length ? ", " : "") + i.perks.join(", ") + (i.item == Item.empty ? "" : ", holding " + i.item));
                for(let j of Game.pups) {
                    if(i.relationWith(j).forth != RelationType.default) {
                        ...
                        if(j.dead) text.insertAdjacentText("beforeend", "†");
                    }
                }
                if(i.kills) text.insertAdjacentText("beforeend", ", " + i.kills + " kill" + (i.kills == 1 ? "" : "s"));
                if(i.killedBy || i.deathCause) text.insertAdjacentText("beforeend", " - killed");
                if(i.killedBy) {
                ...
                if(i.deathCause) text.insertAdjacentText("beforeend", " (" + i.deathCause + ")");
