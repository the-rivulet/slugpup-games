...
import { getId, Stat, Action, Item, shuffle, Relation, DeathCause, RelationType, Perk, Creature, randomColor, choice } from "./globals.js";
...
export class Game {
    ...
    static actionPool: Action[] = [
        ...
        {pups: 2, text: "[0] shares some knowledge with [1].", chance: 0.3, bias: t => t[0].friend(0.3), req: t => !t[0].anyRelation(t[1], RelationType.swornEnemies), effect: t => {if(Math.random() < 0.5 && !t[0].anyRelation(t[1], RelationType.friends)) t[0].become(RelationType.friends, t[1], true);}},
        ...
        {pups: 1, text: "[0] starts to question [0:their] own sanity.", chance: 0.1, bias: t => t[0].bias(Stat.nervousness, 0.1), req: t => !t[0].perks.includes(Perk.suicidal), effect: t => {if(Math.random() < 0.5) t[0].perks.push(Perk.suicidal);}},
        ...
        // Scavs
        {pups: 1, text: "[0] trades [0:their] pearl for a scavenger's grenade.", chance: 0.6, bias: t => t[0].perks.includes(Perk.highRep) ? 0.9 : 0, req: t => t[0].item == Item.pearl && !t[0].perks.includes(Perk.lowRep), effect: t => {t[0].item == Item.grenade; if(Math.random() < 0.5 && !t[0].perks.includes(Perk.highRep)) t[0].perks.push(Perk.highRep);}},
        {pups: 2, text: "[0] purchases [R]s for [0:themselves] and [0:their] friend [1] with a pearl.", chance: 0.8, bias: t => t[0].perks.includes(Perk.highRep) ? 1.2 : 0, req: t => t[0].item == Item.pearl && t[0].relationWith(t[1]).forth == RelationType.friends && !t[0].perks.includes(Perk.lowRep) && !t[1].perks.includes(Perk.lowRep),
            effect: t => {let x = choice([Item.spear, Item.spear, Item.grenade, Item.grenade, Item.flashbang]); t[0].item = x; t[1].item = x; if(Math.random() < 0.5 && !t[0].perks.includes(Perk.highRep)) t[0].perks.push(Perk.highRep); if(Math.random() < 0.3 && !t[1].perks.includes(Perk.highRep)) t[1].perks.push(Perk.highRep); return x;}},
        {pups: 1, text: "[0] throws a spear into a scavenger toll in an act of defiance.", chance: 0.3, bias: t => t[0].attack(0.3), req: t => t[0].item == Item.spear && !t[0].perks.includes(Perk.highRep), effect: t => {t[0].item = Item.empty; t[0].xp += 6; if(!t[0].perks.includes(Perk.lowRep)) t[0].perks.push(Perk.lowRep);}},
        {pups: 1, text: "[0] spears a scavenger to death so [0:they] can pass the toll without paying.", chance: 0.3, bias: t => t[0].attack(0.3), req: t => t[0].item == Item.spear && !t[0].perks.includes(Perk.highRep), effect: t => {t[0].item = Item.empty; t[0].xp += 6; if(!t[0].perks.includes(Perk.lowRep)) t[0].perks.push(Perk.lowRep);}},
        {pups: 2, text: "[0] and [1] don't want to pay, so they run past opposite sides of a scavenger toll and barely escape.", chance: 0.3, bias: t => t[0].attack(0.3) + t[1].attack(0.3), req: t => !t[0].perks.includes(Perk.highRep) && !t[1].perks.includes(Perk.highRep), effect: t => {for(let i of t) {if(!i.perks.includes(Perk.lowRep)) i.perks.push(Perk.lowRep);}}},
        {pups: 2, text: "[0] and [1] try to pass a scavenger toll without paying. Unforunately, [1] doesn't make it.", chance: 0.3, bias: t => t[0].attack(0.3) + t[1].attack(0.3) + t[1].dodge(-0.3), req: t => !t[0].perks.includes(Perk.highRep) && !t[1].perks.includes(Perk.highRep), effect: t => {if(!t[0].perks.includes(Perk.lowRep)) t[0].perks.push(Perk.lowRep); t[1].die(DeathCause.spearThrow, new Creature("Scavenger", "#ccc"));}},
        {pups: 1, text: "[0] runs past a scavenger toll without paying and barely escapes with [0:their] life.", chance: 0.3, bias: t => t[0].attack(0.3), req: t => !t[0].perks.includes(Perk.highRep), effect: t => {if(!t[0].perks.includes(Perk.lowRep)) t[0].perks.push(Perk.lowRep);}},
    ...
    static nextRound() { try {
        ...
        for(let i of order) {
            ...
            let output: void | string;
            if(i.action.action.effect) output = i.action.action.effect(i.action.pups);
            i.output = output || "";
            i.hasInitiated = true;
        ...
        setTimeout(function() { try {
            ...
            for(let i of Game.pups.sort((a, b) => a.realKills == b.realKills ? b.xp - a.xp : b.realKills - a.realKills)) {
                ...
                let text = document.createElement("span");
                if(i.dead) text.style.color = "#999";
                ...
                if(!i.dead) text.insertAdjacentText("beforeend", " - alive" + (i.item == Item.empty ? "" : " - holding " + i.item));
