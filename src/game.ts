...
import { getId, Stat, Action, Item, shuffle, Relation, DeathCause, RelationType, Perk } from "./globals.js";
    ...
    static nextRound() { try {
        ...
        setTimeout(function() { try {
            ...
            for(let i of Game.pups) {
                ...
                if(i.dead && !i.killedBy && !i.deathCause) text.textContent += " - dead";
                if(!i.dead) text.textContent += " - alive" + (i.perks.length ? ", " : "") + i.perks.join(", ");
                for(let j of Game.pups) {
                    if(i.relationWith(j).forth != RelationType.default) text.innerHTML += `, ${j.dead ? "formerly " : ""}${i.relationWith(j).forth} with <img src="./assets/slugpup.png" class="pupimg tiny" style="background: ${j.color};"> ${j.name}${j.dead ? "†" : ""}`;
                }
                if(i.kills) text.textContent += ", " + i.kills + " kill" + (i.kills == 1 ? "" : "s");
                if(i.killedBy || i.deathCause) text.textContent += " - killed";
                if(i.killedBy) text.textContent += " by " + i.killedBy.name;
                if(i.deathCause) text.textContent += " from " + i.deathCause;
